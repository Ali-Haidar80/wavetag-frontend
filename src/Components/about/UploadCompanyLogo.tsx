...
const UploadCompanyLogo:React.FC<{inputs: AboutInputTypes,setInputs: Dispatch<SetStateAction<AboutInputTypes>>}> = ({inputs, setInputs}) => {
...
const handleDragEnter = (e: DragEvent<HTMLDivElement>) => {
  ...
  const handleFiles = (file: File) => {
    setInputs({...inputs,companyLogo: file})
    setUploaded(true)
  };
 ...
 <>
    ...
    {
        uploaded ?  <small className='text-primary-200 text-[8px] md:text-[10px]'>{inputs.companyLogo?.name}</small>:
